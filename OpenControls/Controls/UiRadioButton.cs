...
public sealed class UiRadioButton : UiElement
{
    ...
    public UiColor SelectedColor { get; set; } = new UiColor(120, 140, 200);
    ...
    private void SetChecked(bool value, bool notify)
    {
        ...
        if (_checked && Parent != null && !string.IsNullOrWhiteSpace(GroupId))
        {
            foreach (UiElement child in Parent.Children)
            {
                if (child is UiRadioButton radio && radio != this && string.Equals(radio.GroupId, GroupId, StringComparison.Ordinal))
                {
                    radio.SetChecked(false, true);
                }
            }
        }
